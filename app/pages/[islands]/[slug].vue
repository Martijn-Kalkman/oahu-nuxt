<template>
  <div class="">
    <div class="relative h-screen">
      ...
      >
        <h1 class="text-4xl font-bold mb-2">{{ category.headerTitle }}</h1>
        <h2 class="text-2xl">{{ category.headerSubTitle }}</h2>
        <a :href="category.slug">Book now</a>
      </div>
    </div>
    ...
    <!--            &lt;!&ndash; <video src="/oahutoursvideo.mp4" autoplay class="mb-6"></video> &ndash;&gt;-->
    <!--            <swiper-->
    <!--              :spaceBetween="-55"-->
    <!--              :centeredSlides="true"-->
    <!--              :slidesPerView="1.3"-->
    ...
    <!--              <div-->
    <!--                v-for="(card, index) in category.cards?.slice(0, 1)"-->
    <!--                :key="card.slug"-->
...
const category = ref(await $fetch(`/api/tours/${route.params.slug}`));
</script>
